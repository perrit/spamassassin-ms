...
Used by the SQLSlaveBasedAddrList storage implementation.

This will set the DSN used to connect to the master database.  Example:
C<DBI:mysql:spamassassin:localhost>

=cut

  push (@cmds, {
		setting => 'user_awl_master_dsn',
		is_admin => 1,
		type => $Mail::SpamAssassin::Conf::CONF_TYPE_STRING
	       });
...
=item user_master_awl_master_sql_username username

Used by the SQLSlaveBasedAddrList storage implementation.

The authorized username to connect to the master DSN.

=cut

  push (@cmds, {
		setting => 'user_awl_master_sql_username',
		is_admin => 1,
		type => $Mail::SpamAssassin::Conf::CONF_TYPE_STRING
	       });
...
The password for the database username, for the above DSN. If left unspecified
user_awl_master_sql_username is used instead.
...
=item user_awl_master_sql_password password

Used by the SQLSlaveBasedAddrList storage implementation.

The password for the master database username, for the master DSN.

=cut

  push (@cmds, {
		setting => 'user_awl_master_sql_password',
		is_admin => 1,
		type => $Mail::SpamAssassin::Conf::CONF_TYPE_STRING
	       });
...
The table user auto-whitelists are stored in, for the above DSN. If left
unspecified user_awl_sql_password is used instead.
...
=item user_awl_master_sql_table tablename

Used by the SQLSlaveBasedAddrList storage implementation.

The table user auto-whitelists are stored in, for the master DSN. If left
unspecified user_awl_sql_table is used instead.

=cut

  push (@cmds, {
		setting => 'user_awl_master_sql_table',
		is_admin => 1,
		default => 'awl',
		type => $Mail::SpamAssassin::Conf::CONF_TYPE_STRING
	       });
